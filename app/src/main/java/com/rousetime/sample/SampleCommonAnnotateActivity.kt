 ...
 */
class SampleCommonAnnotateActivity : AppCompatActivity() {
    ...
    private fun start() {
        ...
        var pair: Pair<Int, StartupSortStore>? = null
        when (intent.getIntExtra("id", -1)) {
            R.id.sync_and_sync -> {
                pair = StartupSortStoreConfig.getSyncAndSyncConfig()
            }
            R.id.sync_and_async -> {
                pair = StartupSortStoreConfig.getSyncAndAsyncConfig()
            }
            R.id.async_and_sync -> {
                pair = StartupSortStoreConfig.getAsyncAndSyncConfig()
            }
            R.id.async_and_async -> {
                pair = StartupSortStoreConfig.getAsyncAndAsyncConfig()
            }
            R.id.async_and_async_await_main_thread -> {
                pair = StartupSortStoreConfig.getAsyncAndAsyncAwaitMainThreadConfig()
            }
            R.id.manual_dispatch -> {
                pair = StartupSortStoreConfig.getManualDispatchConfig()
            }
            R.id.thread_priority -> {
                pair = StartupSortStoreConfig.getthreadPriorityConfig()
            }
            R.id.multiply_process -> {
                bindService(
                    Intent(this, MultipleProcessAnnotateService::class.java),
        ...
        Handler().postDelayed({
            StartupManager.Builder()
                .setConfig(config)
                .apply {
                    if (pair != null) setStartupSortStore(
                        pair.second,
                        pair.first
                    ) else addAllStartup(list)
                }
                .build(this)
                .start()
                .await()
